...
function Navbar() {
  ...
  const measure = () => {
    ...
    setPillRect({
      left: linksBox.left - rowBox.left,
      width: linksBox.width,
    });

    setFullRect({
      left: logoBox.left - rowBox.left,
      width: authBox.right - logoBox.left,
    });

    setHeight(linksBox.height + 24); // padding around the links
  };
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      ...
      >
        ...
        <div
          className="absolute rounded-full bg-paper/90 backdrop-blur-md shadow-sm transition-all duration-500 ease-out"
          style={{
            left: `${active.left}px`,
